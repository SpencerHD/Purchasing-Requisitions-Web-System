<?php
    ...
    * description : This file displays an open requisition for requesters to be
	* able to change certain information and upload additional files, as well as,
	* delete them depending on the status of the requisition.
    * ---------------------------------------------------------------------------
    */
...
<body>
	...
	<hr style="width:98%">
    <div class="container">    		
	    <form action="open.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
			<div class="row">
				<div class="col-25">
			...
			<div class="row">
				...
				<div class="col-75">
					<label for="progress" id="progressLabel"><?php echo $data['progress'];?></label>
					<select id="progress" name="progress" class="select">
						<option selected="selected"><?php echo $data['progress'];?></option>
						<option value="<?php echo empty($progress)?$progress:'Re-Submitted';?>">Re-Submitted</option>
					</select>
				</div>
		    </div>
			<div class="row">
			...
			<div class="row">
				...
				<div class="col-75">
					<?php
						...
						$confirm_text = "Are you sure you want to delete this attachment?";
						$id = $data['id'];
						...
						foreach ($pdo->query($sql) as $row) {
							...
							if ($data['progress'] != "Rejected" && $data['assigned'] != "Unassigned") {
								echo '<a class="link" href="uploads/' . $data['ds'] . "/" . $row['name'] . '" download><span>' . substr($row['name'],0,30) . '</span></a>';
							} else {
								echo '<a class="link" href="uploads/' . $data['ds'] . "/" . $row['name'] . '" download><span>' . substr($row['name'],0,30) . '</span></a><a href="deletefile.php?id='.$id.'&attach_id='.$attach_id.'" id="delete" class="custom-delete" onclick="return clicked();"><span><i class="fa fa-trash"></i></span></a>&ensp;&ensp;';
							}
						}
					...
					<?php
						...
						foreach ($pdo->query($sql) as $row) {
							...
							if ($data['progress'] != "Rejected" && $data['assigned'] != "Unassigned") {
								...
								echo '<a class="link" href="uploads/' . $data['ds'] . "/" . $row['name'] . '" download><span>' . substr($row['name'],0,30) . '</span></a><a href="deletefile?id='.$id.'&attach_id='.$attach_id.'" id="delete" class="custom-delete" onclick="return clicked();"><span><i class="fa fa-trash"></i></span></a>&ensp;&ensp;';
							}
						}
			...
			<div class="row">
				<div class="col-25">
					<label class="expcomment">Add Comment:</label>
				</div>
				<div class="col-75">
	...
	<script>
		var progress = "<?php echo $data['progress'] ?>";
		if (progress == "Submitted" || progress == "Rejected") {
