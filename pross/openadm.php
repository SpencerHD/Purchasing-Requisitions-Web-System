<?php
    ...
    * description : This file displays an open requisition for the admin only
	* where the admin can change any information and add their own comment to the
	* current selected requisition.
    * ---------------------------------------------------------------------------
    */
	...
	if (!empty($_POST)) {
		...
		if ($program != null) {
			...
			$sql = "UPDATE reqs SET program = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($program,$id));
		}		
		if ($type != null) {
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE reqs SET type = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
		...
		if ($submitted != null) {
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE reqs SET submitted = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($submitted,$id));
		}
		if ($assigned != null) {
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		if ($expcomment != null) {
			...
			$sql = "INSERT INTO comments(reqs_id, name, comment) VALUES ('$reqs_id', '$logedInUsername', '$comment')";
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$q = $pdo->prepare($sql);
			$q->execute(array());
		}
...
<body>
	...
	<hr style="width:98%">
    <div class="container">    		
	    <form action="openadm.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
			<div class="row">
				<div class="col-25">
			...
			<div class="row">
				...
				<div class="col-75">
					<select id="progress" name="progress" class="select">
						...
						<option value="<?php echo empty($progress)?$progress:'Submitted';?>">Submitted</option>
			...
			<div class="row">
				...
				<div class="col-75">
					<select name="submitted" class="select">
						<option selected="selected"><?php echo $data['submitted'];?></option>
						<?php
							$pdo = Database::connect();
							
							$sql = "SELECT * FROM users WHERE role LIKE 'requester'";
				...
				<div class="col-75">
					<?php
						...
						foreach ($pdo->query($sql) as $row) {
							...
							echo '<a class="link" href="uploads/' . $data['ds'] . "/" . $row['name'] . '" download><span>' . substr($row['name'],0,30) . '</span></a><a href="deletefile.php?id='.$id.'&attach_id='.$attach_id.'&loc=adm" class="custom-delete" id="delete" onclick="return clicked();"><span><i class="fa fa-close"></i></span></a>&ensp;&ensp;';
						}
			...
			<div class="row">
				...
				<div class="col-75">
					<textarea id="expcomment" name="expcomment" data-emojiable="true" placeholder="New comment..." style="height:55px" value="<?php echo !empty($expcomment)?$expcomment:'';?>"></textarea>
				</div>
			</div>
			<div class="row">
