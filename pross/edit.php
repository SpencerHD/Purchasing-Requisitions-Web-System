<?php 
    ...
    * description : This file edits a user's information within the admin screen
	* and updates it.
    * ---------------------------------------------------------------------------
    */
	...
	if (!empty($_POST)) {
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "UPDATE users SET name = ?, last_name = ?, role = ?, product_line = ? WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($name,$last_name,$role,$product_line,$id));
		Database::disconnect();
		header("Location: users.php");
	} else {
		$pdo = Database::connect();
...
<body>
    ...
    <div class="tmp">
		<div class="tmp">
			...
			<form class="tmp" action="edit.php?id=<?php echo $id?>" method="post">
				<div class="tmp">
				...
				<br>
				<div class="tmp">
					<label class="control-label" style="font-size: 18px">Product Line</label><br>
					<input name="product_line" type="text" style="width: 400px" placeholder="Product Line" required oninput="this.value = this.value.replace(/[0-9.]/g, '')" maxlength="20" value="<?php echo !empty($product_line)?$product_line:'';?>">
				</div>
				<br>
				<button type="button" class="button button1" style="background-color: grey" onclick="location.href='users.php';"><span>Cancel</span></button>
				<button type="submit" class="button button2"><span>Update</span></button>
			</form>
		</div>
